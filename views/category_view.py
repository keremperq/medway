...
def equipments_by_category_page(category_id):
    db = current_app.config["db"]
    category = db.category.get_row(where_columns="CATEGORY_ID", where_values=category_id)
    if request.method == "GET":
        equipment_category_list = db.category.get_table(where_columns="CATEGORY_ID", where_values=category_id)
        equipments = []
        ...
        return render_template("equipment/equipments.html", equipments=equipments, title="Equipments in category of " + category.category_name)
    else:
