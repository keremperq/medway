...
def equipments_by_category_page(cat_id):
    ...
    if request.method == "GET":
        equipment_category_list = db.equipment.get_table(where_columns="CAT_ID", where_values=cat_id)
        equipments = []
        for equipment_category in equipment_category_list:
            equipments.append((db.equipment.get_row(equipment_category.eq_id), take_categories_by_equipment(equipment_category.eq_id)))
